...
components.html(
    ...
    style.innerHTML = `
        ...
        .hero-title {
            animation: floatUp 2s cubic-bezier(0.2, 0.8, 0.2, 1) forwards !important;
        }
...
def render_message_payload(msg):
    ...
    if msg.get("images"):
        st.markdown("</br>##### 🖼️ Visual Archive Plates", unsafe_allow_html=True)
        ...
        for idx, img_path in enumerate(msg["images"]):
            ...
            with cols[idx % len(cols)]:
                   # Render using the resolved backend URL or local path
                    st.image(target, caption=f"Plate: {plate_stem}")
